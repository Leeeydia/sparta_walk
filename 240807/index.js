...
async function fetchComments() {
  try {
    ...
    slicedData.forEach((comment) => {
      ...
      //    comment의 body의 길이가 40 이상일 때   (if문) if(comment.body.length >= 40){}
      //    0번째부터 39번째 문자를 복사하고 맨 뒤에 "..."을 붙인 문자열을 slicedBody = comment.body.slice(0,40) + "..."
      //    slicedBody라는 변수에 할당해주세요.

      // 조건문,,,,,, if문으로 코맨트 바디 길이가 40 이상일때 여기기
